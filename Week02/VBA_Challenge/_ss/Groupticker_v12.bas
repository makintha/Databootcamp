...
Sub GroupTicker()
    ...
    Dim in_opening As Integer, in_closing As Integer
...
'                Stock_Total = Stock_Total + ws.Cells(i, 7).Value
        ...
        For j = 2 To NTicker
            If j = 2 Then
                in_opening = j
                Opening = ws.Cells(in_opening, 3).Value
            ...
            If Opening = 0 And Closing = 0 Then
                ...
                ' Warning Message
                MsgBox ("WARNING!:" & vbCrLf & "This Worksheet has incomplete dataset" & vbCrLf & "The Problem data is at Ticker = " & ws.Cells(j, 9).Value)
                ws.Cells(6, 15).Value = "WARNING INCOMPLETE DATASET" & vbCrLf & "CHECK DATASET FOR TICKER = " & ws.Cells(j, 9).Value
                ws.Cells(6, 15).Font.ColorIndex = 3
                ws.Range("I" & j & ":L" & j).Font.ColorIndex = 3
            ' Missing few data in the beginning
            ElseIf Opening = 0 And Closing > 0 Then
                ' Search where opening value not 0
                st = in_opening + 1
                For k = st To in_closing
                    next_val = ws.Cells(k + 1, 3).Value
                    cur_val = ws.Cells(k, 3).Value
                    If ws.Cells(k + 1, 3).Value <> ws.Cells(k, 3).Value Then
                        new_op_index = k + 1
                        Opening = ws.Cells(new_op_index, 3).Value
                        Exit For
                    End If
                Next k
