...
'                Stock_Total = Stock_Total + ws.Cells(i, 7).Value
    ...
    Range("K2:K" & NTicker).NumberFormat = "0.00%"
    ...
    'Find min/max value
    Minimum = Application.WorksheetFunction.Min(Range("K2:K" & NTicker))
    ws.Cells(3, 17).Value = Minimum
    
    Maximum = Application.WorksheetFunction.Max(Range("K2:K" & NTicker))
    ws.Cells(2, 17).Value = Maximum
    
    Max_Vol = Application.WorksheetFunction.Max(Range("L2:L" & NTicker))
